...
def rooms_ids_for_booking(date_from: date, date_to: date, hotel_id: int | None = None):
    rooms_count = (
        ...
        .filter(BookingsOrm.date_from <= date_to, BookingsOrm.date_from >= date_from)
    ...
    rooms_ids_from_hotel = rooms_ids_from_hotel.subquery()
    ...
    rooms_ids_to_get = (
        ...
        .filter(
            ...
            rooms_left_table.c.room_id.in_(rooms_ids_from_hotel),
        )
    )
