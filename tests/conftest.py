...
async def setup_db(check_mode):
    ...
    hotels_ = json_to_model("tests/mock_hotels.json", HotelAdd)
    rooms_ = json_to_model("tests/mock_rooms.json", RoomAdd)
...
async def test_root(ac, setup_db):
    await ac.post(
        "/auth/register", json={"email": "pupis@pes.com", "password": "pupis123"}
    )
...
async def authenticated_ac(test_root, ac):
    await ac.post(
        "/auth/login", json={"email": "pupis@pes.com", "password": "pupis123"}
    )
